...
| 下载输出 | `Ctrl/⌘ + S`（格式化视图） |
| Enter | 保留原行缩进，括号/匹配 XML 标签内自动添加一级缩进 |
| Tab / Shift+Tab | 插入缩进、选区多行缩进 / 反缩进；候选弹出时 Tab 接受补全 |
| 上下方向键 / Enter / Esc | 选择、接受 / 关闭补全候选 |
| Backspace | 光标位于成对的括号或引号之间时删除整对 |
